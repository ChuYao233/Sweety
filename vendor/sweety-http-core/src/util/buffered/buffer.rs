...
use tracing::trace;
use sweety_io_compat::bytes::{Buf, BytesMut};
use sweety_collection::bytes::{BufList, ChunkVectoredUninit, read_buf};

pub use sweety_io_compat::bytes::{BufInterest, BufRead, BufWrite};
...
impl<const LIMIT: usize> ReadBuf<LIMIT> {
    #[inline(always)]
    pub fn new() -> Self {
        // 预分配 8KB：覆盖 99% 的 HTTP/1.1 请求头大小，避免首次读触发扩容
        Self(BytesMut::with_capacity(8192))
    }
...
#[derive(Default)]
pub struct WriteBuf<const LIMIT: usize>(sweety_io_compat::bytes::WriteBuf);

impl<const LIMIT: usize> WriteBuf<LIMIT> {
    #[inline]
    pub fn new() -> Self {
        Self(sweety_io_compat::bytes::WriteBuf::new())
    }
...
// buf list 最大长度：64 支持最多 32 个 pipeline 请求各带一个 body 块
// 相比原来 32，在高并发 pipeline 场景减少背压次数，提升吞吐
const BUF_LIST_CNT: usize = 64;
