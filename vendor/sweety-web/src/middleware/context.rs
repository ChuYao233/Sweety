...
/// middleware for bridging `sweety-http-core` and `sweety-web` types. useful for utilizing sweety-web
/// when manually constructing http services.
///
/// # Examples
/// ```rust
/// use sweety_http_core::util::{
///     middleware::context::ContextBuilder,
///     service::{handler::handler_service, route::get, Router}
/// };
/// use sweety_service::{Service, ServiceExt};
/// use sweety_web::{error::Error, handler::state::StateRef};
///
/// // handler function depending on sweety-web types.
/// async fn handler(StateRef(_): StateRef<'_, ()>) -> Error {
///     sweety_web::error::ErrorStatus::bad_request().into()
/// }
///
/// # async fn test() {
/// // low level router from sweety-http-core
/// let router = Router::new()
///     // utilize handler function with sweety-web types
///     .insert("/", get(handler_service(handler)))
///     // utilize sweety-web's middleware to sweety-http-core's router
///     .enclosed(sweety_web::middleware::CatchUnwind)
///     // converting sweety-http-core types to sweety-web types with web context middleware.
///     .enclosed(sweety_web::middleware::WebContext)
///     // sweety-http-core's context builder middleware is also needed for helping type conversion.
///     // this middleware is tasked with constructing typed application state and pass down it
///     // to sweety_web's WebContext middleware. In case of absence of application state a tuple
...
///     .unwrap();
///
/// assert_eq!(res.status(), sweety_http_core::http::StatusCode::BAD_REQUEST);
/// # }
/// ```
...
mod service {
    ...
    use sweety_http_core::util::middleware::context::Context;
