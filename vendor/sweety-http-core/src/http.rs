...
/// # Example
/// ```rust
/// # use sweety_http_core::http::{Request, Response};
...
///     use sweety_http_core::http::IntoResponse;
...
pin_project! {
    ...
    pub struct RequestExt<B> {
        ...
        // sweety tries to be strongly typed when possible. runtime type casting is meant for library
...
/// used for foreign types (from sweety-http-core pov) that can be impl with [Borrow] trait.
pub trait BorrowReq<T> {
    fn borrow(&self) -> &T;
}

/// trait for Borrow &mut T from &mut Self.
/// used for foreign types (from sweety-http-core pov) that can be impl with [BorrowMut] trait.
