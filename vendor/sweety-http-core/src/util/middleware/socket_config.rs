...
use tracing::warn;
use sweety_io_compat::net::{Stream as ServerStream, TcpStream};
use sweety_service::{Service, ready::ReadyService};

#[cfg(unix)]
use sweety_io_compat::net::UnixStream;
...
impl SocketConfig {
    pub const fn new() -> Self {
        Self {
            ka: None,
            // 默认开启 TCP_NODELAY，对标 Nginx tcp_nodelay on
            // 禁用 Nagle 算法，HTTP 响应头立即发送，降低 TTFB
            nodelay: true,
        }
    }
