...
fn compare(c: &mut Criterion) {
    ...
    let mut sweety = sweety_router::Router::new();
    for route in register!(colon) {
        sweety.insert(route, true).unwrap();
    }
    group.bench_function("sweety", |b| {
        b.iter(|| {
            for route in black_box(call()) {
                black_box(sweety.at(route).unwrap());
            }
        });
    });
...
fn compare_erase(c: &mut Criterion) {
    ...
    group.bench_function("sweety", |b| {
        b.iter(|| {
            for route in black_box(call()) {
                black_box(sweety.at(route).unwrap().params);
            }
        });
    });
...
fn compare_empty_path(c: &mut Criterion) {
    ...
    let mut sweety = sweety_router::Router::new();
    sweety.insert("/", true).unwrap();
    group.bench_function("sweety", |b| {
        b.iter(|| black_box(sweety.at(black_box("/")).unwrap()));
    });
