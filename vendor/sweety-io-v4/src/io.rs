...
pub trait AsyncIo: io::Read + io::Write + Unpin {
    ...
    ///
    /// This constraint is from `tokio`'s behavior which is what sweety built upon and rely on
    /// in downstream crates like `sweety-http-core` etc.
    ...
    ///
    /// # Why:
    /// std `can_vector` feature is not stabled yet and sweety make use of vectored io write.
...
/// use sweety_io::io::{AsyncIo, AsyncRead, AsyncWrite, PollIoAdapter, ReadBuf};
///
