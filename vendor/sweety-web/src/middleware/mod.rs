//! middleware types.
//!
//! Middleware in sweety-web is powered by [`Service`] and [`ServiceExt`] trait.
...
//! ```rust
//! # use sweety_web::{error::Error, http::WebResponse, service::Service, WebContext};
...
//! ```rust
//! # use sweety_web::{error::Error, handler::handler_service, http::WebResponse, service::Service, App, WebContext};
//! // from above example we added generic type C to WebContext and Error type and C is type of application state.
//! // in sweety-web application state is compile time type checked therefore in middleware you have to annotate
//! // the type to comply.
...
//! // in sweety-web http body is a dynamic type that can be mutated and transformed and marking it generic
...
//! ```rust
//! # use sweety_web::{error::Error, handler::handler_service, http::WebResponse, service::Service, App, WebContext};
//! // in sweety-web a named type middleware is constructed with a two step process:
...
pub use sweety_http_core::util::middleware::Extension;
pub use sweety_service::middleware::{AsyncFn, Group, UncheckedReady};

#[cfg(test)]
mod test {
    use sweety_http_core::body::RequestBody;
    use sweety_collection::futures::NowOrPanic;
