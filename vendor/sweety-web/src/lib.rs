...
//! # Quick start
//! ```no_run
//! use sweety_web::{handler::handler_service, route::get, App};
//!
...
//! }
//!
//! // Almost all magic extract types in sweety-web utilize zero copy
...
//!     assert_eq!(method, Method::GET);
//!     // blocking thread for long period of time does not impact sweety-web
//!     // async internal.
...
//! # fn _main() -> std::io::Result<()> {
//! // ServiceExt::enclosed_fn combinator enables async function as middleware.
//! // in sweety_web almost all service can be enclosed by an middleware.
//! App::new()
//!     .at("/",
//!         get(
...
//! use tower-http inside sweety-web application.
//! ```rust
...
//!
//! ## opt-in proc macro
//! in sweety-web proc macro is opt-in. This result in a fast compile time with zero
//! public proc macro. That said you still can enable macros for a higher level style
//! of API.
//! ```rust
//! # #[cfg(feature = "codegen")]
//! # async fn _main() -> std::io::Result<()> {
//! use sweety_web::{codegen::route, App};
//!
//! #[route("/", method = get)]
...
pub mod http {
    ...
    pub use sweety_http_core::http::*;

    /// type alias for default request type sweety-web uses.
    pub type WebRequest<B = RequestBody> = Request<RequestExt<B>>;

    /// type alias for default response type sweety-web uses.
...
pub mod route {
    ...
    //! # }
    //! ```
    pub use sweety_http_core::util::service::route::{Route, connect, delete, get, head, options, patch, post, put, trace};
}
