...
{
    #[cfg(unix)]
    /// transform Self to a http1 service builder that producing a service that able to handle [sweety_io_compat::net::UnixStream]
    pub fn unix(
        self,
    ) -> HttpServiceBuilder<marker::Http1, sweety_io_compat::net::UnixStream, FA, HEADER_LIMIT, READ_BUF_LIMIT, WRITE_BUF_LIMIT>
    where
        FA: Service,
    {
        HttpServiceBuilder {
    ...
    /// transform Self to a http1 service builder that producing a service that able to handle [sweety_io_compat::net::io_uring::TcpStream]
    pub fn io_uring(
        self,
    ) -> HttpServiceBuilder<
        marker::Http1Uring,
        sweety_io_compat::net::io_uring::TcpStream,
        FA,
        HEADER_LIMIT,
        READ_BUF_LIMIT,
        WRITE_BUF_LIMIT,
