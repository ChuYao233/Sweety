pub use sweety_router::{MatchError, params::Params};

use core::{fmt, marker::PhantomData};

use std::{collections::HashMap, error};

use sweety_service::{BoxFuture, FnService, Service, object::BoxedServiceObject, pipeline::PipelineT};
...
///
/// An [ServiceObject](sweety_service::object::ServiceObject) must be specified as a type parameter
    ...
    Arg: Clone,
{
    ...
    async fn call(&self, arg: Arg) -> Result<Self::Response, Self::Error> {
        let mut router = sweety_router::Router::new();
...
mod service {
    use sweety_service::ready::ReadyService;
    ...
    pub struct RouterService<S> {
        ...
        pub(super) router: sweety_router::Router<S>,
    }
        ...
        Req: BorrowReq<Uri> + BorrowReqMut<Params>,
    {
        ...
        fn call(&self, mut req: Req) -> impl Future<Output = Result<Self::Response, Self::Error>> {
            async {
                ...
                let sweety_router::Match { value, params } =
...
mod test {
    ...
    use sweety_service::{ServiceExt, fn_service, ready::ReadyService};
    ...
    fn router_typed() {
        ...
        impl TypedRoute for Index {
            ...
            fn route() -> Self::Route {
                Req::into_object(RouterMapErr(sweety_service::fn_service(func)))
            }
        }
