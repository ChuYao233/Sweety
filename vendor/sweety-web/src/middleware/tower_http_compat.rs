//! compatibility between tower-http layer and sweety-web middleware.

use tower_layer::Layer;

use crate::service::{Service, tower_http_compat::TowerCompatService};

/// A middleware type that bridge `sweety-service` and `tower-service`.
/// Any `tower-http` type that impl [Layer] trait can be passed to it and used as sweety-web's middleware.
///
/// # Type mutation
...
impl<L> TowerHttpCompat<L> {
    /// Construct a new sweety-web middleware from tower-http layer type.
    ///
    /// # Limitation:
    ...
    /// # use sweety_web::{http::{StatusCode, WebResponse}, service::fn_service, App, WebContext};
    /// use sweety_web::middleware::tower_http_compat::TowerHttpCompat;
