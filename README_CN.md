...
- 📁 **静态文件** — 内存缓存 + Range + ETag/Last-Modified + `try_files` + sendfile(2) 零拷贝
...
- **P99 延迟**：H1 1KB 114ms vs 691ms（**−83%**）；H2 1KB 376ms vs 853ms（**−56%**）
- **内存占用**：启动 **8.65 MB** vs 75.34 MB（**−88%**），多数负载下减少 44–79%
- **零错误**：所有测试场景零请求失败
- Nginx 在 100KB–1MB H2 场景凭 `sendfile(2)` 内核零拷贝占优
